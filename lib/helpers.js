...
var word = require('enwiktionary-analyzer').word;
...
exports.wordLinkHref = wordLinkHref;
...
exports.eachExplanationSegment = eachExplanationSegment;
...
function wordLinkHref(wordLink) {
  return new Handlebars.SafeString('/'+wordLink[1]+'/'+wordLink[0]);
}
...
function eachExplanationSegment(explanation, options) {
  var fragments = explanation.toFragmentsAndWordLinks();

  var ret = "";

  for(var i=0, j=fragments.length; i<j; i++) {
    var c = word.isWordLink(fragments[i])? {mentionedWord:fragments[i]} : {text:fragments[i]};
